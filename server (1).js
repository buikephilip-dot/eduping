...
// Twilio signature validation — rejects any request not genuinely from Twilio
function validateTwilioSignature(req, res, next) {
  if (!hasTwilio()) return next(); // skip in demo mode
  const twilioSignature = req.headers['x-twilio-signature'];
  const url = `${process.env.BASE_URL || `https://${req.headers.host}`}${req.originalUrl}`;
  ...
  if (!isValid) {
    ...
    return res.status(403).type('text/xml').send('<Response><Message>Forbidden</Message></Response>');
  }
  next();
}
...
// JWT_SECRET is required — no fallback. Server will not start without it.
...
async function seedIfEmpty() {
  ...
  const school = await q(`INSERT INTO schools
    ...
    ['Greenfield Academy', 'Abuja', 'Green gate beside the assembly hall', '85000 per term', '15th of each term month', '2nd Term 2024/2025', '+2347015255068', process.env.TWILIO_WHATSAPP_NUMBER || '+14155238886', hashedPassword, 'starter', 'active', 50000, 100000]);
...
async function getSchoolByTwilio(to) {
  ...
  return result.rows[0] || null; // never guess the tenant
}
...
async function requireSchool(req, res, next) {
  ...
  if (!school) return bad(res, 'Unauthorized school admin', 401);
  // Support both bcrypt hashes (new) and plain text (legacy migration period)
  const passwordMatch = school.admin_password.startsWith('$2')
    ? await bcrypt.compare(password, school.admin_password)
    : school.admin_password === password;
  if (!passwordMatch) return bad(res, 'Unauthorized school admin', 401);
  req.school = school;
  next();
}
...
app.post('/webhook/whatsapp', validateTwilioSignature, (req, res, next) => handleIncomingWhatsApp(req, res).catch(next));
...
app.post('/api/super/schools', requireSuper, async (req, res) => {
  const b = req.body;
  const rawPassword = b.admin_password || uuid().slice(0, 8);
  ...
  const r = await q(`INSERT INTO schools (name,city,landmark_description,fees,fee_deadline,current_term,whatsapp_number,twilio_number,admin_password,plan,status,billing_start,monthly_retainer,setup_fee)
    ...
    [b.name,b.city,b.landmark_description||'',b.fees||'',b.fee_deadline||'',b.current_term||'',b.whatsapp_number||'',b.twilio_number||null,hashedPassword,b.plan || 'starter',b.billing_start || new Date(),b.monthly_retainer || 0,b.setup_fee || 0]);
  // Return plain password once so admin can share it with school — never stored plain again
  json(res, { ...r.rows[0], admin_password_plain: rawPassword }, 201);
});
...
app.post('/api/admin/login', async (req, res) => {
  const r = await q('SELECT id,name,city,status,admin_password FROM schools WHERE id=$1', [req.body.school_id]);
  const school = r.rows[0];
  if (!school) return json(res, { ok: false, school: null });
  // Support both bcrypt hashes (new) and plain text (legacy migration period)
  const passwordMatch = school.admin_password.startsWith('$2')
    ? await bcrypt.compare(req.body.password, school.admin_password)
    : school.admin_password === req.body.password;
  const { admin_password, ...safeSchool } = school;
  json(res, { ok: passwordMatch, school: passwordMatch ? safeSchool : null });
});
...
(async () => {
  ...
  if (!process.env.JWT_SECRET) throw new Error('JWT_SECRET is required. Add a strong random string to Railway environment variables.');
  await migrate();
  await seedIfEmpty();
